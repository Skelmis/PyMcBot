...
def nbt_to_snbt(tag):
    ...
    if type(tag) in arrays:
        return "[" + arrays[type(tag)] + ";" + ",".join(map(repr, tag.value)) + "]"
    ...
    if isinstance(tag, pynbt.TAG_Compound):
        return "{" + ",".join(n + ":" + nbt_to_snbt(v) for (n, v) in tag.items()) + "}"
...
class JoinGamePacket(AbstractDimensionPacket):
    ...
    get_definition = staticmethod(
        lambda context: [
            ...
            if context.protocol_version >= 730
            else {},
            {"world_names": PrefixedArray(VarInt, String)}
            if context.protocol_version >= 722
            else {},
            {"dimension_codec": NBT} if context.protocol_version >= 718 else {},
            {
                "dimension": NBT
                if context.protocol_version >= 748
                else String
                if context.protocol_version >= 718
                else Integer
                if context.protocol_version >= 108
                else Byte
            },
            ...
            {
                "max_players": VarInt
                if context.protocol_version >= 749
                else UnsignedByte
            },
