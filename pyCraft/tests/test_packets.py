...
class PacketListenerTest(unittest.TestCase):
    def test_listener(self):
        ...
        for protocol_version in TEST_VERSIONS:
            ...
            packet = serverbound.play.ChatPacket(context).set_values(message=message)
...
class PacketEnumTest(unittest.TestCase):
    def test_packet_str(self):
        class ExamplePacket(Packet):
            id = 0x00
            packet_name = "example"
            definition = [{"alpha": VarInt}, {"beta": VarInt}, {"gamma": VarInt}]
...
class TestReadWritePackets(unittest.TestCase):
    ...
    def test_combat_event_packet(self):
        ...
        self.assertEqual(str(packet), "CombatEventPacket(event=EnterCombatEvent())")
        self._test_read_write_packet(packet)
    ...
    def test_spawn_object_packet(self):
        for protocol_version in TEST_VERSIONS:
            ...
            self.assertEqual(
                ...
                "data=1, velocity_x=21, velocity_y=55, velocity_z=41)" % packet.id
                if protocol_version >= 100
                else "0x%02X SpawnObjectPacket(entity_id=49846, "
                "object_uuid='d9568851-85bc-4a10-8d6a-261d130626fa', "
                "type_id=EGG, x=68, y=38, z=76, pitch=180, yaw=263.494, "
                "data=1, velocity_x=21, velocity_y=55, velocity_z=41)" % packet.id
                if protocol_version >= 49
                else "0x%02X SpawnObjectPacket(entity_id=49846, type_id=EGG, "
            ...
            self._test_read_write_packet(
                packet, context, yaw=360 / 256, pitch=360 / 256
            )
            self._test_read_write_packet(
                packet2, context, yaw=360 / 256, pitch=360 / 256
            )
    ...
    def test_sound_effect_packet(self):
        for protocol_version in TEST_VERSIONS:
            ...
            packet = clientbound.play.SoundEffectPacket(
                sound_id=545, effect_position=Vector(0.125, 300.0, 50.5), volume=0.75
            )
    ...
    def test_face_player_packet(self):
        for protocol_version in TEST_VERSIONS:
            ...
            self.assertEqual(
                ...
                if protocol_version >= 353
                else "0x%02X FacePlayerPacket(entity_id=None, x=1.0, y=-2.0, z=3.5)"
                % packet.id,
            )
            self._test_read_write_packet(packet, context)
            ...
            self.assertEqual(
                ...
                if protocol_version >= 353
                else "0x%02X FacePlayerPacket(entity_id=123)" % packet.id,
            )
            self._test_read_write_packet(packet, context)
        ...
        if context is None:
            ...
            for packet_attr, precision in kwargs.items():
                ...
                self.assertAlmostEqual(
                    packet_attribute_in, packet_attribute_out, delta=precision
                )
